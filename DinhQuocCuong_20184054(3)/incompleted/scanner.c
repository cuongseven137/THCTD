...
Token* readNumber(void) {
  ...
  if (tk->string[i-1] == CHAR_PERIOD) {
    error(ERR_INVALID_SYMBOL, lineNo, colNo);
  }

  if (count == 0) {
    tk->tokenType = TK_NUMBER;
    tk->intValue = atoi(tk->string);
  } else if (count == 1) {
    tk->tokenType = TK_FLOAT;
    tk->floatValue = (float)atof(tk->string);
  } else {
    error(ERR_INVALID_SYMBOL, lineNo, colNo);
  }
  return tk;
...
Token* readPlus() {
  ...
  if (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_EQ) {
      readChar();
      return makeToken(SB_ASSIGN_PLUS, ln, col);
    }
  }
  return makeToken(SB_PLUS, ln, col);
}
...
Token* readMinus() {
  ...
  if (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_EQ) {
      readChar();
      return makeToken(SB_ASSIGN_SUBTRACT, ln, col);
    }
  }
  return makeToken(SB_MINUS, ln, col);
}
...
Token* readTimes() {
  ...
  if (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_EQ) {
      readChar();
      return makeToken(SB_ASSIGN_TIMES, ln, col);
    }
  }
  return makeToken(SB_TIMES, ln, col);
}
...
Token* readSlash() {
  ...
  if (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_EQ) {
      readChar();
      return makeToken(SB_ASSIGN_DIVIDE, ln, col);
    }
  }
  return makeToken(SB_SLASH, ln, col);
}
...
Token* getToken(void) {
  ...
  case CHAR_PLUS: return readPlus();
