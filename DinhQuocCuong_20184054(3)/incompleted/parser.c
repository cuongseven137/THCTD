...
void compileForSt(void) {
  ...
  varType = compileLValue();
  if (varType->typeClass == TP_FLOAT) {
    error(ERR_FLOAT_FOR_INDEX, currentToken->lineNo, currentToken->colNo);
  }
  eat(SB_ASSIGN);
...
Type* compileExpression3(Type* argType1) {
  ...
  case SB_PLUS:
    eat(SB_PLUS);
    if (argType1->typeClass != TP_INT && argType1->typeClass != TP_FLOAT)
      checkIntType(argType1);
    argType2 = compileTerm();
    if (argType2->typeClass != TP_INT && argType2->typeClass != TP_FLOAT)
...
Type* compileTerm2(Type* argType1) {
  ...
  case SB_MODULUS:
    eat(SB_MODULUS);
    if (argType1->typeClass != TP_INT)
      error(ERR_INVALID_MODUL_OPERATOR,  lookAhead->lineNo, lookAhead->colNo);
    argType2 = compileFactor();
    if (argType2->typeClass != TP_INT) {
      error(ERR_INVALID_MODUL_OPERATOR,  lookAhead->lineNo, lookAhead->colNo);
    }
    resultType = compileTerm2(argType1);
    break;
